...
class ThreadPageViewScreen extends StatefulWidget {
  final String title;
  final int postCount;
  final int threadId;
  final int page;
  ...
  @override
  _ThreadPageViewScreenState createState() => _ThreadPageViewScreenState();
}

class _ThreadPageViewScreenState extends State<ThreadPageViewScreen>
    with
        AfterLayoutMixin<ThreadPageViewScreen>,
  ...
  void onTapEditPost(BuildContext context, ThreadPost post) async {
    ...
    if (result != null) {
      ...
      await refreshPage();
      print('Do the scroll');
      _pageViewController.jumpTo((details.posts.length - 1).toDouble());
    }
  }

  void swapPageListener() {
    if (scrollController.offset >
        scrollController.position.maxScrollExtent + 170) {
      print('Swipe down?');
    ...
    if (scrollController.offset <
        scrollController.position.minScrollExtent - 170) {
      _pageViewController.animateToPage(_pageViewController.page.toInt() + 0,
          duration: Duration(milliseconds: 200), curve: Curves.easeInQuart);
    }
  }
